...
module Util.BMP exposing (bmp24)
...
bmp24 : Int -> Int -> List Int -> String
bmp24 w h pixels =
    bmp24With w h pixels { defaultColor = 0x00FFFF00, order = RightDown }
...
bmp24With : Int -> Int -> List Int -> { a | defaultColor : Int, order : Order } -> String
bmp24With w h pixels { defaultColor, order } =
    let
        -- TODO convert int->base64 string on build
        result =
            [ 0x42, 0x4D ]
